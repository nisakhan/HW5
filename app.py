...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/nisakhanHW5"
...
class TodoListForm(FlaskForm):
    ...
    items = TextAreaField("Enter your TODO list items in the following format: Description, Priority")
...
class PriorityofaTodDoList(FlaskForm):
    newitemspriority = StringField("What is the priority of this item?", validators=[Required()])
    submit = SubmitField("Update")
...
def update(item):
    ...
    form = PriorityofaTodDoList()
    if form.validate_on_submit():
        toto = TodoItem.query.filter_by(description = item).first()
        toto.priority = form.newitemspriority.data
        db.session.commit()
...
def delete(lst):
    ...
    todoq = TodoList.query.filter_by(title=lst).first()
    db.session.delete(todoq)
    db.session.commit()
    flash("Delete list {}".format(lst))
    return redirect(url_for('all_lists'))
